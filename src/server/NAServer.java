...
public class NAServer implements DPSSInterface{
	/**
	...
	synchronized public String getPlayerStatus(String username, String password, String ip) {
		...
		if (adminObj.userName.equals("Admin") && adminObj.password.equals("Admin")) {
			...
			output += DatafromOtherIP(username, password, ip, Constants.EU_SERVER_PORT, "getPlayerStatus");
			output += " ";
			output += DatafromOtherIP(username, password, ip, Constants.AS_SERVER_PORT, "getPlayerStatus");
			return output;
		}
		...
		while (true) {
			try {
				...
				if (fun.trim().equals("getPlayerStatus")) {
					...
					temp = createPlayerAccount(firstName, lastName, age, username, password, ip);
				}
